...
class DAGScheduler(
  ...
  def submitJob[T, U](
    ...
    // 生成JobId,应用CAS,保证线程安全
    val jobId = nextJobId.getAndIncrement()
    if (partitions.size == 0) { // 分区为0,表肯没有任务,直接返回
      // Return immediately if the job is running 0 tasks
      return new JobWaiter[U](this, jobId, 0, resultHandler)
    }

    assert(partitions.size > 0) // 验证分区数
   ...
   * they arrive.
   *  // 运行一个action任务,将运行结果传递给resultHandler
   * @param rdd target RDD to run tasks on // 运行任务目标RDD
   * @param func a function to run on each partition of the RDD // 运行RDD中每一个任务的函数
   * @param partitions set of partitions to run on; some jobs may not want to compute on all
   *    partitions of the target RDD, e.g. for operations like first()
    *  // RDD分区的集合,有些job不需要运行所有的RDD任务,如:first()
   * @param callSite where in the user program this job was called // 户程序的job运行调用位置
   * @param resultHandler callback to pass each result to // 结果回调函数
   * @param properties scheduler properties to attach to this job, e.g. fair scheduler pool name
    *                   // 此job的scheduler配置,如fair scheduler pool name
   *
